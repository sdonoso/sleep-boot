...
from .serializers import DataSerializer, PersonSerializer
...
class InfoDataPerson(View):
    ...
    def get(self, request, id_telegram, *args, **kwargs):
        ...
        data = Data.objects.filter(person=person,
                                   time_stamp__range=[today - datetime.timedelta(days=7), today]).order_by('time_stamp')
...
class DateToday(View):
    def get(self, request, id_telegram, *args, **kwargs):
        ...
        today = timezone.now()
        data = Data.objects.filter(person=person, time_stamp__day=today.day, time_stamp__year=today.year,
